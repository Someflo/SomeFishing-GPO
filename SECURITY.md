...
Los saltos opcionales usan Espacio sin direcciones. Las compras usan E, los puntos marcados, Ctrl+A, Retroceso y dígitos, y pueden gastar Peli del juego. La selección de cebo añade un clic en el punto marcado para cada tipo. La recuperación puede pulsar No/Cancelar o cerrar los tres puntos. No se leen textos del portapapeles ni se escriben textos libres.
...
La interfaz usa tres puntos marcados, uno para cada tipo. Mueve el puntero gradualmente, espera 350 ms al llegar, envía un clic y espera su liberación. No necesita leer la fila ni el borde amarillo; completar el clic no verifica que el juego haya seleccionado ese tipo. Solo usa cantidades estimadas superiores a uno y reserva una unidad de cada tipo para evitar que desaparezcan sus filas. Si se modifican los cebos fuera de la macro o cambian las posiciones, hay que corregir cantidades y puntos.

El OCR opcional compara el contador de la fila activa con el inventario estimado. Su zona es independiente y opcional; sin ella, informa cantidad desconocida. Al cambiar de tipo, descarta resultados pendientes del lector anterior para no mezclar cantidades. El OCR no reemplaza por sí solo lo escrito por el usuario ni confirma un pedido. Usa Windows OCR local y referencias pequeñas de x2/x3/x4; no descarga idiomas. Una lectura espera como máximo 8 segundos, con 3 segundos por reconocimiento. Si Windows no termina una operación cancelada, conserva una sola operación pendiente y sus recursos hasta que termine, sin acumular trabajos nuevos.
...
Tras una fase fallida, una secuencia independiente identifica el menú y cancela con el botón derecho o cierra los tres puntos. No abre E, escribe cantidades ni compra durante esa limpieza. Solo permite regresar a la preparación de pesca tras observar ausencia estable. Con recuperación activada, puede repetir la limpieza tras una pausa, con un límite de fallos consecutivos; una compra que pudo enviarse sigue bloqueando nuevas compras si su resultado es incierto.
...
Recuperar fallos temporales viene activado al migrar la interfaz. Las pérdidas prolongadas de seguimiento liberan el clic y esperan la barra o el cierre; no usan coordenadas antiguas. Hay pausas crecientes y un máximo ajustable de fallos seguidos. En ese modo, el tope de compras cuenta órdenes enviadas, incluso inciertas, y no intentos fallidos anteriores a Comprar. Guardar ajustes usa un único escritor en segundo plano y una instantánea pendiente reemplazable; un fallo de disco aparece en la interfaz y no detiene el control de pesca. Al cerrar se espera un tiempo limitado para guardar: un cierre forzado puede perder los cambios recientes.
